...
document.addEventListener("DOMContentLoaded", function(e){
  ...
  function clickHandler(){
    document.addEventListener('click', function(e){
      if(e.target.matches('.up-vote')){
        ...
        movieAdapter.update(movieId, body)
        .then(movieObj => {
  ...
  const getMovies = () => {
    ...
    movieAdapter.getAll()
    .then(movieObjects => {
      const movies = movieObjects.map(movieObj => {
      ...
      Movie.renderCollection(movies, movieList)
    })
  }
...
/*
Adapter class
- it should abstract enough to work with different resources
- it should work for different APIs
